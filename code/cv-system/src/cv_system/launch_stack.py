"""Start IDE relay and cv-system with one command."""
...
def main() -> None:
    ...
    health = os.environ.get("CV_STACK_HEALTH_URL", f"http://{host}:{port}/api/v1/health")

    relay = subprocess.Popen([sys.executable, "-m", "cv_system.bridge.ide_relay_server"])
    rc = 1
    try:
        if not _wait_for_relay(health):
            print(f"cv-stack: IDE relay did not respond at {health}", file=sys.stderr)
            raise SystemExit(1)
        print("cv-stack: IDE relay OK; starting cv-system...")
        rc = subprocess.call([sys.executable, "-c", "from cv_system.main import main; main()"])
    finally:
        relay.terminate()
        try:
