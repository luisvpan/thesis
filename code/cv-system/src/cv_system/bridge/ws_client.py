...
class WebSocketBridge:
    ...
    Attributes:
        url: WebSocket server URL (e.g., ws://localhost:8765/live).
    ...
    def __init__(
        ...
        if url is None:
            url = os.getenv("LANGUAGE_RUNTIME_WS_URL", "ws://localhost:8765/live")
        ...
        # True after disconnect(): avoids reconnect tasks during process shutdown.
        self._shutdown = False
        # threading.get_ident() of the asyncio loop thread (set by run_websocket_client).
    ...
    async def _graceful_shutdown(self) -> None:
        """Run on ``bridge.loop``: close socket, then stop the loop (no bare ws=None first)."""
        try:
    ...
    def disconnect(self) -> None:
        """
        Disconnect from non-async code (e.g. main ``finally``).

        Schedules ``await ws.close()`` on the bridge's event loop so the
        websockets library does not schedule work after ``loop.stop()`` /
        ``loop.close()`` (avoids ``RuntimeError: Event loop is closed``).
        """
...
# Aliases for tests and shorter imports
ConnectionRefusedError = WebSocketConnectionRefusedError
ConnectionClosedError = WebSocketConnectionClosedError