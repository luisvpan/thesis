...
def calculate_dmax(device, calibrated_area, xv_min, yv_min, xv_max, yv_max, num_frames=500):
    ...
    for _ in tqdm(range(num_frames), desc="Numero de frames", unit="frames"):
        ...
        depth_data = np.frombuffer(frame.get_buffer_as_uint16(), dtype=np.uint16).reshape(depth_camera_resolution[0], depth_camera_resolution[1])
...
def detectar_cuadrados_blancos(frame):
    print("Detectando cuadrados blancos...")
...
def calibrar_mesa_y_detectar_toques(device):
    ...
    while True:
        ...
        cuadrados_detectados = detectar_cuadrados_blancos(frame_rgb)
        ...
        if len(cuadrados_detectados) >= 2:
            ...
            if len(puntos_camara) == 2:
                ...
                xw_min = max(0, min(xw_min, color_camera_resolution[1]))
                xw_max = max(0, min(xw_max, color_camera_resolution[1]))
                yw_min = max(0, min(yw_min, color_camera_resolution[0]))
                yw_max = max(0, min(yw_max, color_camera_resolution[0]))
                ...
                # Aplicar escalado
                xw_min = xw_min - 10
                xw_max_escalado = min(color_camera_resolution[1], int(xw_centro + (xw_max - xw_centro) * factor_escala_x))
