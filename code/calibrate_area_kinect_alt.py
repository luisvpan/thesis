...
# Intentar importar diferentes bibliotecas de Kinect
try:
    from pykinect2 import PyKinectV2
    from pykinect2 import PyKinectRuntime
    KINECT_LIBRARY = "pykinect2"
    print("Usando pykinect2")
except ImportError:
    try:
        import freenect2
        KINECT_LIBRARY = "freenect2"
        print("Usando freenect2")
    except ImportError:
        try:
            from freenect import sync_get_depth, sync_get_video
            KINECT_LIBRARY = "freenect"
            print("Usando freenect (Kinect v1)")
        except ImportError:
            print("ERROR: No se encontró ninguna biblioteca de Kinect compatible")
            print("Instala una de las siguientes:")
            print("  pip install pykinect2")
            print("  pip install freenect2")
            print("  pip install freenect")
            exit(1)
...
# Clase para manejar diferentes bibliotecas de Kinect
class KinectHandler:
    def __init__(self):
        self.kinect = None
        self.library = KINECT_LIBRARY
        self.initialize_kinect()
    
    def initialize_kinect(self):
        if self.library == "pykinect2":
            try:
                self.kinect = PyKinectRuntime.PyKinectRuntime(
                    PyKinectV2.FrameSourceTypes_Color | PyKinectV2.FrameSourceTypes_Depth
                )
                print("Kinect v2 inicializada con pykinect2")
            except Exception as e:
                print(f"Error al inicializar Kinect v2: {e}")
                raise
        elif self.library == "freenect2":
            try:
                self.kinect = freenect2.Freenect2()
                print("Kinect v2 inicializada con freenect2")
            except Exception as e:
                print(f"Error al inicializar Kinect v2: {e}")
                raise
        elif self.library == "freenect":
            print("Kinect v1 inicializada con freenect")
    
    def get_color_frame(self):
        if self.library == "pykinect2":
            if self.kinect.has_new_color_frame():
                frame = self.kinect.get_last_color_frame()
                frame = frame.reshape((1080, 1920, 4))  # Kinect v2: 1920x1080, 4 canales (BGRA)
                frame = cv2.cvtColor(frame, cv2.COLOR_BGRA2BGR)
                frame = cv2.resize(frame, (640, 480))  # Redimensionar para compatibilidad
                return frame
        elif self.library == "freenect2":
            # Implementar para freenect2
            pass
        elif self.library == "freenect":
    ...
    def get_depth_frame(self):
        if self.library == "pykinect2":
            if self.kinect.has_new_depth_frame():
                ...
                frame = frame.reshape((424, 512))  # Kinect v2: 512x424
                frame = cv2.resize(frame, (640, 480))  # Redimensionar para compatibilidad
                return frame
        elif self.library == "freenect2":
            # Implementar para freenect2
            ...
            frame, _ = sync_get_depth()
            frame = cv2.resize(frame, (640, 480))
            return frame
        return None
    
    def close(self):
        if self.kinect and hasattr(self.kinect, 'close'):
            try:
                self.kinect.close()
            except:
                pass
...
def calculate_dmax(kinect_handler, calibrated_area, xv_min, yv_min, xv_max, yv_max, num_frames=500):
    ...
    for _ in tqdm(range(num_frames), desc="Numero de frames", unit="frames"):
        ...
        depth_data = kinect_handler.get_depth_frame()
...
def calibrar_mesa_y_detectar_toques():
    ...
    while True:
        ...
        # Obtener frames
        frame_bgr = kinect_handler.get_color_frame()
        if frame_bgr is not None:
            ...
            if len(cuadrados_detectados) >= 2:
                ...
                if len(puntos_camara) == 2:
                    ...
                    # Obtener frame de profundidad para extraer la ROI escalada
                    depth_data = kinect_handler.get_depth_frame()
                    if depth_data is not None:
                        depth_data = cv2.flip(depth_data, 1)
    ...
    # Calcular dmax_map
    dmax_map = calculate_dmax(kinect_handler, (xw_min, yw_min_escalado, xw_max_escalado - xw_min, yw_max - yw_min_escalado), xv_min, yv_min, xv_max, yv_max)
    ...
    if 'xw_min' in locals():
        ...
        if preguntar_usuario("¿Desea proceder con la detección de toques?"):
            # Calcular dmax_map
            dmax_map = dmax_map - 4
            ...
            while True:
                ...
                if depth_data is not None:
                    ...
                    # Obtener frame de color para visualización
                    frame_bgr = kinect_handler.get_color_frame()
                    if frame_bgr is not None:
                        frame_bgr = cv2.flip(frame_bgr, 1)
    ...
    # Liberar recursos de Kinect
    kinect_handler.close()

if __name__ == "__main__":
    calibrar_mesa_y_detectar_toques()
