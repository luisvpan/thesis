...
# Función para obtener frame de color de Kinect v2 usando freenect2
def get_color_frame(kinect):
    try:
        color_frame = kinect.get_color_frame()
        if color_frame is not None:
            # freenect2 devuelve frames en formato BGRA, convertir a BGR
            frame = cv2.cvtColor(color_frame, cv2.COLOR_BGRA2BGR)
            frame = cv2.resize(frame, (640, 480))  # Redimensionar para compatibilidad
            return frame
    except Exception as e:
        print(f"Error obteniendo frame de color: {e}")
    return None

# Función para obtener frame de profundidad de Kinect v2 usando freenect2
def get_depth_frame(kinect):
    try:
        depth_frame = kinect.get_depth_frame()
        if depth_frame is not None:
            # freenect2 devuelve frames de profundidad en formato float32
            # Convertir a uint16 para compatibilidad
            depth_frame = (depth_frame * 1000).astype(np.uint16)
            frame = cv2.resize(depth_frame, (640, 480))  # Redimensionar para compatibilidad
            return frame
    except Exception as e:
        print(f"Error obteniendo frame de profundidad: {e}")
    return None

# Función para calcular dmax_map con barra de progreso y mensaje
def calculate_dmax(kinect, calibrated_area, xv_min, yv_min, xv_max, yv_max, num_frames=500):
    ...
    for _ in tqdm(range(num_frames), desc="Numero de frames", unit="frames"):
        # Obtener frame de profundidad usando freenect2
        depth_data = get_depth_frame(kinect)
...
def calibrar_mesa_y_detectar_toques():
    ...
    # Inicializar Kinect v2 usando freenect2
    try:
        ...
        kinect.start()
        print("Kinect v2 inicializada correctamente con freenect2")
    except Exception as e:
    ...
    while True:
        ...
        # Obtener frames usando freenect2
        frame_bgr = get_color_frame(kinect)
    ...
    # Calcular dmax_map
    dmax_map = calculate_dmax(kinect, (xw_min, yw_min_escalado, xw_max_escalado - xw_min, yw_max - yw_min_escalado), xv_min, yv_min, xv_max, yv_max)
