...
function renderGlyph(item: ResultVisualItem, index: number) {
  ...
  switch (item.kind) {
    ...
    case 'forma':
      return <FormaGlyph key={key} subtype={item.subtype} />;
    case 'cap':
...
export function ResultArrayVisual({ items, grouping }: ResultArrayVisualProps) {
  if (items.length === 0) return null;

  const shown = items.slice(0, MAX_SHOW);
  const overflow = items.length - shown.length;

  // Without grouping: flat render as before
  if (!grouping) {
    return (
  ...
  return (
    <div className="mt-2 flex w-full max-w-52 flex-col items-center gap-1">
