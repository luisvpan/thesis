...
function readDigitValue(data: SourceFlowNodeData): number {
...
export function getNumberTouchGroups(nodes: DataflowNode[]): NumberTouchGroup[] {
  ...
  for (const members of byRoot.values()) {
    ...
    const digits = sorted.map((n) =>
      readDigitValue(n.data as SourceFlowNodeData)
    );
...
export function isNumberMergeHead(
  ...
  return (
    data.variant !== "number" ||
    !data.numberMergePrimaryId ||
    data.numberMergePrimaryId === nodeId
  );
}
...
export function isNumberMergeTail(
  ...
  return (
    data.variant !== "number" ||
    !data.numberMergeTailId ||
    data.numberMergeTailId === nodeId
  );
}

function numberDataEqual(a: SourceFlowNodeData, b: SourceFlowNodeData): boolean {
  return (
    a.variant === b.variant &&
    a.value === b.value &&
...
export function applyNumberTouchMerge(nodes: DataflowNode[]): DataflowNode[] {
  ...
  const next = nodes.map((node) => {
    ...
    const prev = node.data as SourceFlowNodeData;
    ...
    const updated: SourceFlowNodeData = {
      ...prev,
      digitValue,
      value: mergedValue,
