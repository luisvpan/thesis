export function UNION(inputs: Array<{ id: string; value: unknown }>): unknown[] {
  ...
  const elements1 = set1.value as unknown[];
  const elements2 = set2.value as unknown[];
...
export function INTERSECTION(inputs: Array<{ id: string; value: unknown }>): unknown[] {
  ...
  const elements1 = set1.value as unknown[];
  const elements2 = set2.value as unknown[];
  return elements1.filter(elem => elements2.includes(elem));
}
...
export function COMPLEMENT(inputs: Array<{ id: string; value: unknown }>): unknown[] {
  ...
  const universalElements = universe.value as unknown[];
  const subsetElements = subset.value as unknown[];
  return universalElements.filter(elem => !subsetElements.includes(elem));
}
